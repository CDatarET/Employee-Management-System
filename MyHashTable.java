...
public class MyHashTable {
	...
	public void displayContents() {
        ...
        public EmployeeInfo removeEmployee(int theEmpNum) {
            ...
            int lastNum = theEmpNum%10;
            for(int i = 0; i < buckets[lastNum].size(); i++){
      if(buckets[lastNum].get(i).empNum == theEmpNum){
        buckets[lastNum].remove(i);
        numInHashTable = numInHashTable -1;
        System.out.println("removed "+theEmpNum);
      }
      else{
          System.out.println("no such employee with that number exists");
      }
    }
            return null;
        }
        ...
        public EmployeeInfo getEmployee(int theEmpNum) {
            ...
            for(int i = 0; i < buckets[lastNum].size(); i++){
      			if(buckets[lastNum].get(i).empNum == theEmpNum){
        			System.out.println("got "+theEmpNum+ " at "+ buckets[lastNum].get(i));
        			return buckets[lastNum].get(i);
      			}
    		}
			EmployeeInfo nullEmployee = new EmployeeInfo(-1,"","",-1,-1,1.0);
			System.out.println("No such employee with that number exists");
            return nullEmployee;
        }
